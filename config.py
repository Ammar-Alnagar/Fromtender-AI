...
DEMO_LIST = [
  ...
  },
  {
    ...
    "title": "Mawared HR Employees Page",
    "description": """Page Layout:

A sidebar navigation panel on the left with sections for "Dashboard," "Employees," "Requests," "Attendance," "Financials," and more.
A top navigation bar for search functionality, user profile options, and notifications.
Employee Section:

A grid or card layout displaying individual employee details like name, department, position, and status (e.g., "Not Signed In," "Day Off," "Present").
Icons for quick actions (e.g., phone, email, edit, and delete).
A filter bar to sort employees by office, manager, position, department, or work timing.
Functionalities:

Dynamic "Add New" button for adding employees.
Filtering options with dropdown menus or checkboxes.
Pagination or infinite scroll for scalability.
Technology Stack:

Frontend: HTML, CSS (Tailwind or Bootstrap), JavaScript (React or Angular).
Backend: Node.js or Python (Django/Flask) for server-side logic.
Database: PostgreSQL or MongoDB for employee data management.
Styling and Theme:

Clean, professional look with consistent spacing and fonts.
Icons and buttons should have tooltips for better accessibility.
Use light and neutral colors for better readability.
Code Standards:

Modular and reusable components.
Adherence to accessibility standards (WCAG).
Clean and commented code for maintainability."""
  },
]